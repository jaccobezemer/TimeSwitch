...
static const char PORTAL_PAGE[] =
    ...
    "<title>TimeSwitch WiFi Instellen</title>"
    "<style>"
    "body{font-family:sans-serif;background:#1a1a2e;color:#ccc;"
    "display:flex;justify-content:center;align-items:center;"
    "min-height:100vh;margin:0}"
    ".box{background:#2a2a3e;padding:2em;border-radius:12px;"
    ...
    "button{background:#00aa44;color:#fff;border:none;"
    "padding:12px 32px;border-radius:6px;font-size:16px;"
    "cursor:pointer;width:100%%}"
    "</style></head><body><div class='box'>"
    "<h2>TimeSwitch</h2>"
    "<p class='sub'>WiFi instellen</p>"
...
static esp_err_t portal_post_handler(httpd_req_t *req)
{
    ...
    if (len <= 0) {
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Geen data");
        return ESP_FAIL;
    }
    body[len] = '\0';
    ...
    static const char SUCCESS_PRE[] =
        ...
        "<style>body{font-family:sans-serif;background:#1a1a2e;color:#ccc;"
        "display:flex;justify-content:center;align-items:center;"
        "min-height:100vh;margin:0}"
        ".box{background:#2a2a3e;padding:2em;border-radius:12px;"
        "text-align:center;max-width:380px;width:90%%}"
        "h2{color:#00aa44;margin-top:0}</style></head><body>"
        "<div class='box'><h2>Opgeslagen!</h2>"
        "<p>Verbinding wordt gemaakt met <strong>";
    static const char SUCCESS_POST[] =
        "</strong>.<br>TimeSwitch herstart nu...</p></div></body></html>";
...
esp_err_t captive_portal_start(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Server starten mislukt: %s", esp_err_to_name(err));
        return err;
    }
    ...
    ESP_LOGI(TAG, "Captive portal gestart — http://192.168.4.1");
    return ESP_OK;
}
